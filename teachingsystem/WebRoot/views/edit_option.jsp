...
<html>
	<head>
		...
		<title>修改选择题</title>
		<%	//获取到内容储存到
		Choice choice = (Choice)request.getAttribute("choice");
	 	%>
	</head>
	...
	<body>
		<div id="bg" >
			...
			<div id="page">
				...
				<span style="margin-left:20px">选项A：</span>
				<input type="text" name="answer" id="chooseA" value=<%=choice.getChooseA() %>>
				<span style="margin-left:20px">答案：</span>
				<select name="answer" style="font-family:'微软雅黑'" id="answer">
					<option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option>
				</select>
				<script type="text/javascript">
					$("#answer option[value=<%=choice.getAnswer()%>]").attr("selected",true);
				</script>
				<br/>
				<span style="margin-left:20px">选项B：</span>
				<input type="text" name="answer" id="chooseB" value=<%=choice.getChooseB() %>><br/>
				<span style="margin-left:20px">选项C：</span>
				<input type="text" name="answer" id="chooseC" value=<%=choice.getChooseC() %>><br/>
				<span style="margin-left:20px">选项D：</span>
				<input type="text" name="answer" id="chooseD" value=<%=choice.getChooseD() %>>
				<br />
				<br />
				...
				<script type="text/javascript" src="../ueditor/ueditor.config.js"></script>   
				    ...
				    <script type="text/javascript"> 
				      ...
				      ,initialContent:'<%=choice.getExplain()%>'   
				  ...
				  var ue2 = UE.getEditor('title',{toolbars:[["undo","redo",'|',"fontfamily","fontsize", '|',
				    "bold","italic","underline","strikethrough",'|',"insertimage",'cleardoc']]   
				      ...
				      ,initialContent:'<%=choice.getQuestion()%>' 
				      });
				</script>
					<input type="button" name="submit" class="submit" value="提交" id="editChoose"
		...
		<script type="text/javascript">
			$("#dan").fadeIn(1000);
			$(function(){
				$("#editChoose").bind("click",function(){
					...
					//待修改
					$.get("updateChoice",
					{"chapter":chapter,"type":type,"title":title,"answer":answer,"explain":explain,"chooseA":chooseA,"chooseB":chooseB,"chooseC":chooseC,"chooseD":chooseD,"id":id},		
					function(data){
						if(data=="true"){
							alert("修改成功");
